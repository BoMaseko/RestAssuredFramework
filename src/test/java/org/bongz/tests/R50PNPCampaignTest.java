...
import org.bongz.database.DatabaseOperations;
...
public class R50PNPCampaignTest extends BaseTest{
	
	@Test
	public void R50PNPCampaignTest(Map<String, String> data) throws Exception{
		...
		Response stressScoreResponse = RequestBuilder.getBuilder()
		...
		String campaignQuery = "Select CMP_ACT_CUST_ID_FK from WLNSIN.CUSTOMER_ACTIVITY_STATUS WHERE CUSTOMER_ID =" + customerNo + " order by ID desc";
		HashMap<String, String> dbData = dbops.getSqlResultInMap(campaignQuery);
		String campaignId = dbData.get("CMP_ACT_CUST_ID_FK");
		
		Response processPayoutResponse = given()
				.log()
				.all()
				.baseUri(FrameworkConstants.getBaseURI())
				.pathParam("campaignId", campaignId)
				.get("/CustomerEngagement/rest/campaignActivity/campaignActivityCustomersForBenefit/{campaignId}");
		
		
		ExtentLogger.info("Milestone completed for " + customerNo + " please check email used for payout!!!");
